...
import PRODUCT_CATEGORY_FIELD from '@salesforce/schema/Product2.Store_Pre_requisite_public__c';
...
export default class B2bProductDescription extends LightningElement {
    ...
    wiredProduct({ data, error }) {
        if(data){
            console.log('Field Values ; ' , data.fields.Store_Pre_requisite_public__c.value);
            this.productDetail = data.fields.Store_Pre_requisite_public__c.value;
        }else if(error) {
            console.log(error);
        }
    }
