...
import saveProductSessionInCartItem from '@salesforce/apex/B2BGetInfo.saveProductSessionInCartItem';//VMO 20211005
 ...
 * Guest Label
 */
 import guestmessage from '@salesforce/label/c.GuestMSG';
 import cartUpdateMessage from '@salesforce/label/c.b2b_MSG_CartMessage_Success';//VMO 20211008
 import cartItemErrorMessage from '@salesforce/label/c.GuestMSG';//VMO 20211008
/**
...
export default class ProductDetails extends LightningElement {
    ...
    label = {
        guestmessage,
        cartUpdateMessage,//VMO 20211008
        cartItemErrorMessage//VMO 20211008
    }
    ...
    get displayableProduct() {
       ...
       // console.log('productdate: ', this.product.data);
        return {
            ...
            description: this.product.data.fields.Description /* this.product.data.fields.ShortDescription__c */,//VMO 20211008: Use Short Description instead of Description //VMO 20211021: B-1447 - Use Description instead of Short Description
            image: {
     ...
     */
    get _isCartLocked() {
        console.log('this.cartSummary test'  , this.cartSummary);
        if(this.cartSummary != null && this.cartSummary != undefined){
            this.cartId = this.cartSummary.cartId;
            this.resetCartStatus(this.cartId);
        }
     ...
     */
    addToCart(event) {
        console.log('productId : ' , this.recordId + 'Quantity ' + event.detail.quantity);
        if(this.resolvedEffectiveAccountId == null){
            ...
            addToCart({
                ...
                cartId : this.cartId,
                sessionCode : event.detail.sessionCode,
                sessionDate : event.detail.sessionDate
            })
                .then((result) => {
                    console.log('result ConnectApi.CommerceCart: ', result);

                    saveProductSessionInCartItem({
                        cartItemId              : result.cartItemId,
                        productSessionDetail    : event.detail.sessionCode,
                        productSessionDate      : event.detail.sessionDate
                    }).then((resultSave) => {
                        console.log('resultSave ConnectApi.CommerceCart: ', resultSave);
                    }).catch((error) => {
                        console.log('resultSave error ConnectApi.CommerceCart: ', error);
                    })
                    ...
                    this.dispatchEvent(
                        new ShowToastEvent({
                            title: this.label.cartUpdateMessage,//VMO 20211008
                            message: null,//VMO 20211008
                            variant: 'success',
                            mode: 'dismissable'
                        })
                    );
                ...
                .catch((e) => {
                    ...
                    this.dispatchEvent(
                        new ShowToastEvent({
                            title: this.label.cartItemErrorMessage,//VMO 20211008
                            message: null,//VMO 20211008
    ...
    resetCartStatus(cartStatusId){
        console.log(' resetCartStatus : ' , cartStatusId);
        resetStatus({
            recordId:cartStatusId
        })
        .then(result => {
    ...
    closeModalRedirect() {
        ...
        _url = _url.substring(0, _url.indexOf('/s/'));
        _url = _url+'/s/login/';
